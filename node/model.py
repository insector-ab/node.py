...
class Node(Base):
    ...
    def _get_child_ids(self, cls=None, group=None, relation_type=None, exclude_subclasses=False, order_by=None):
        tuples = self._get_related_node_id_query( Edge.CHILD, cls, group, relation_type, exclude_subclasses, order_by).all()
        return [tup[0] for tup in tuples]
    ...
    def _get_parent_ids(self, cls=None, group=None, relation_type=None, exclude_subclasses=False, order_by=None):
        tuples = self._get_related_node_id_query( Edge.PARENT, cls, group, relation_type, exclude_subclasses, order_by ).all()
        return [tup[0] for tup in tuples]
